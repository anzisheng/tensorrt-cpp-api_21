...
FaceEmbdding_trt2::FaceEmbdding_trt2(string onnxModelPath, const YoloV8Config &config)
{
    ...
    m_trtEngine_embedding2 = std::make_unique<Engine<float>>(options);
    ...
    auto succ = m_trtEngine_embedding2->buildLoadNetwork(onnxModelPath, SUB_VALS, DIV_VALS, NORMALIZE);
    if (!succ) {
...
std::vector<float> FaceEmbdding_trt2::detect(cv::Mat& srcimg,        std::vector<cv::Point2f>& face_landmark_5)
{
    // first crop_img, then GPU
    //cout << "ect"<<endl;
    Mat crop_img;
    warp_face_by_face_landmark_5(srcimg, crop_img, face_landmark_5, this->normed_template, Size(96, 96));
...
std::vector<float> FaceEmbdding_trt2::detect(cv::cuda::GpuMat &inputImageBGR,  std::vector<cv::Point2f>& face_landmark_5)
{
    ...
    auto succ = m_trtEngine_embedding2->runInference(input, featureVectors);
    //cout << "after runInference"<<endl;

    //
    const auto &numOutputs = m_trtEngine_embedding2->getOutputDims().size();
...
std::vector<float> FaceEmbdding_trt2::postprocess(std::vector<float> &featureVector)
{
    const auto &outputDims = m_trtEngine_embedding2->getOutputDims();
    auto numChannels = outputDims[0].d[1];
    cout << "embedding2 numChannels size:"<<numChannels<<endl;
...
std::vector<std::vector<cv::cuda::GpuMat>> FaceEmbdding_trt2::preprocess(const cv::cuda::GpuMat &gpuImg, const vector<Point2f> face_landmark_5)
{
    
    // Populate the input vectors
    const auto &inputDims = m_trtEngine_embedding2->getInputDims();
