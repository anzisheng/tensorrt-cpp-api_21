...
Mat paste_back(Mat temp_vision_frame, Mat crop_vision_frame, Mat crop_mask, Mat affine_matrix)
{
    ...
    Mat inverse_matrix;
    cout << "invertAffineTransfo h and w" << affine_matrix.rows<< " , " <<affine_matrix.cols <<endl;
...
bool SampleOnnxMNIST::verifyOutput(const samplesCommon::BufferManager& buffers)
{
    ...
    // ofstream destFile2("swapface_output_mnist.txt", ios::out); 
    ...
    fstream srcFile("swapface_output_mnist.txt", ios::in);
    if(!srcFile.is_open())
    {
    cout << "cann't open the wapface_output.txt"<<endl;
    }
    ...
    // cv::Mat affine_matrix = imread("affine_matrix.jpg");
    // cv::Mat box_mask = imread("box_mask.jpg");

    // std::cout << "*++++++++++ " << endl;
    // box_mask.setTo(0, box_mask < 0);
	// box_mask.setTo(1, box_mask > 1);
    // cv::Mat target_img = imread("target_img.jpg");
    // Mat dstimg = paste_back(target_img, result, box_mask, affine_matrix);
    // imwrite("resultuuuuu.jpg", dstimg);
